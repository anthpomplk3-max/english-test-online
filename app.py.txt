...
class EnglishTestOnline:
    ...
    def initialize_session_state(self):
        ...
        if 'test_duration' not in st.session_state:
            st.session_state.test_duration = 90 * 60  # 90 phút = 5400 giây
    ...
    def display_timer(self):
        ...
        if not st.session_state.test_completed and not st.session_state.time_up:
            elapsed_time = datetime.now() - st.session_state.start_time
            elapsed_seconds = elapsed_time.total_seconds()
            remaining_time = max(0, st.session_state.test_duration - elapsed_seconds)
            ...
            progress = (st.session_state.test_duration - remaining_time) / st.session_state.test_duration
    ...
    def display_sidebar(self):
        ...
        with st.sidebar:
            ...
            # Phần đã làm
            st.subheader("📋 Phần Đã Làm")
            parts_completed = []
            if any(k.startswith('part1_') for k in st.session_state.answers.keys()):
                parts_completed.append("✅ Part 1")
            else:
                parts_completed.append("❌ Part 1")
            
            if any(k.startswith('part2_') for k in st.session_state.answers.keys()):
                parts_completed.append("✅ Part 2")
            else:
                parts_completed.append("❌ Part 2")
            
            if any(k.startswith('part3_') for k in st.session_state.answers.keys()):
                parts_completed.append("✅ Part 3")
            else:
                parts_completed.append("❌ Part 3")
            
            if any(k.startswith('part4_') for k in st.session_state.answers.keys()):
                parts_completed.append("✅ Part 4")
            else:
                parts_completed.append("❌ Part 4")
            
            for part in parts_completed:
                st.write(part)
            ...
            st.session_state.show_explanation = st.checkbox(
                ...
                help="Hiển thị giải thích tại sao đáp án đúng/sau khi chọn đáp án"
            )
            ...
            with col1:
                if st.button("🔄 Bắt đầu lại", use_container_width=True):
                    self.reset_test()
            with col2:
                if st.button("📊 Kết thúc thi", use_container_width=True, type="primary"):
                    st.session_state.test_completed = True
                    st.rerun()
            ...
            with st.expander("📖 Hướng dẫn sử dụng"):
                st.markdown("""
                1. **Làm bài theo thứ tự** từ Part 1 đến Part 4
                2. **Chọn đáp án** bằng cách click vào ô tròn
                3. **Bật/tắt giải thích** ở mục Cài Đặt
                4. **Thời gian**: 90 phút cho 120 câu
                5. **Nộp bài**: Click "Kết thúc thi" hoặc đợi hết giờ
                6. **Xem kết quả**: Sau khi nộp bài
                """)
    
    def reset_test(self):
        """Reset toàn bộ bài thi"""
        for key in ['answers', 'score', 'current_part', 'test_completed', 'time_up']:
    ...
    def display_question(self, question, question_key):
        ...
        with st.container():
            ...
            # Tạo radio buttons hoặc selectbox
    ...
    def calculate_score(self):
        ...
        # Kiểm tra tất cả các câu hỏi
        all_questions = []
        
        # Part 1
        for q in self.part1_questions:
            all_questions.append((f"part1_{q['id']}", q['answer']))
        
        # Part 2
        for passage_key, passage_data in self.part2_passages.items():
            for q in passage_data['questions']:
                all_questions.append((f"part2_{passage_key}_{q['id']}", q['answer']))
        
        # Part 3
        for passage in self.part3_passages:
            for q in passage['questions']:
                all_questions.append((f"part3_{passage['id']}_{q['id']}", q['answer']))
        
        # Part 4
        for q in self.part4_questions:
            all_questions.append((f"part4_{q['id']}", q['answer']))
        
        # Đếm số câu đúng
        for question_key, correct_answer in all_questions:
            if question_key in st.session_state.answers:
                total_answered += 1
                if st.session_state.answers[question_key] == correct_answer:
                    score += 1
        
        return score, total_answered, len(all_questions)
    
    def calculate_part_score(self, part_prefix):
        """Tính điểm cho một phần cụ thể"""
        score = 0
        total = 0
        
        # Tạo danh sách câu hỏi cho phần này
        part_questions = []
        
        if part_prefix == 'part1':
            for q in self.part1_questions:
                part_questions.append((f"part1_{q['id']}", q['answer']))
        elif part_prefix == 'part2':
            for passage_key, passage_data in self.part2_passages.items():
                for q in passage_data['questions']:
                    part_questions.append((f"part2_{passage_key}_{q['id']}", q['answer']))
        elif part_prefix == 'part3':
            for passage in self.part3_passages:
                for q in passage['questions']:
                    part_questions.append((f"part3_{passage['id']}_{q['id']}", q['answer']))
        elif part_prefix == 'part4':
            for q in self.part4_questions:
                part_questions.append((f"part4_{q['id']}", q['answer']))
        
        # Tính điểm
        for question_key, correct_answer in part_questions:
            total += 1
            if question_key in st.session_state.answers:
                if st.session_state.answers[question_key] == correct_answer:
                    score += 1
    ...
    def display_results(self):
        ...
        score, total_answered, total_questions = self.calculate_score()
        ...
        st.markdown(f"# {score}/{total_answered}")
        ...
        st.markdown(f"**⏰ Thời gian làm bài:** {minutes:02d}:{seconds:02d}")
        ...
        part_prefixes = ['part1', 'part2', 'part3', 'part4']
        
        for i, (col, part_name, prefix) in enumerate(zip(cols, parts, part_prefixes)):
            with col:
                part_score, part_total = self.calculate_part_score(prefix)
        ...
        for q in self.part1_questions:
            all_questions_data.append((f"part1_{q['id']}", q, "Part 1"))
        
        for passage_key, passage_data in self.part2_passages.items():
            for q in passage_data['questions']:
                all_questions_data.append((f"part2_{passage_key}_{q['id']}", q, "Part 2"))
        
        for passage in self.part3_passages:
            for q in passage['questions']:
                all_questions_data.append((f"part3_{passage['id']}_{q['id']}", q, "Part 3"))
        
        for q in self.part4_questions:
            all_questions_data.append((f"part4_{q['id']}", q, "Part 4"))
        
        # Tìm câu sai
        for question_key, question_data, part_name in all_questions_data:
            if question_key in st.session_state.answers:
                user_answer = st.session_state.answers[question_key]
        ...
        if wrong_answers:
            for i, wrong in enumerate(wrong_answers[:10]):  # Giới hạn hiển thị 10 câu
                with st.expander(f"{wrong['part']} - Câu {wrong['question_id']}: Đáp án của bạn: {wrong['user_answer']}, Đáp án đúng: {wrong['correct_answer']}"):
                    st.write(f"**Câu hỏi:** {wrong['question']}")
                    st.write(f"**Đáp án của bạn:** ❌ {wrong['user_answer']}")
                    st.write(f"**Đáp án đúng:** ✅ {wrong['correct_answer']}")
                    if wrong['explanation']:
                        st.write(f"**Giải thích:** {wrong['explanation']}")
            
            if len(wrong_answers) > 10:
                st.info(f"Còn {len(wrong_answers) - 10} câu sai khác. Hãy ôn tập kỹ hơn!")
        else:
            st.success("🎉 Bạn không có câu nào trả lời sai!")
        
        # Nút tải kết quả
        st.markdown("---")
        self.download_results(score, total_answered, percentage, minutes, seconds, wrong_answers)
    
    def download_results(self, score, total_answered, percentage, minutes, seconds, wrong_answers):
        """Tạo và tải kết quả"""
        results = {
            "Thời gian thi": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "Thời gian làm bài": f"{minutes:02d}:{seconds:02d}",
            "Tổng số câu": total_answered,
            "Số câu đúng": score,
            "Tỷ lệ đúng": f"{percentage:.1f}%",
            "Đáp án": st.session_state.answers,
            "Câu sai": [
                {
                    'part': w['part'],
        ...
        with col1:
            st.download_button(
                ...
                data=json_data,
                file_name=f"ket_qua_tieng_anh_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                mime="application/json",
                use_container_width=True
            )
        
        with col2:
            if st.button("🔄 Làm bài thi mới", use_container_width=True, type="primary"):
                self.reset_test()
    
    def run_app(self):
        """Chạy ứng dụng chính"""
        # Header
        st.markdown('<div class="main-header">🏥 BÀI THI TIẾNG ANH BẬC 2</div>', unsafe_allow_html=True)
        st.markdown('<div class="sub-header">SỞ Y TẾ TỈNH GIA LAI - NĂM 2025</div>', unsafe_allow_html=True)
        ...
        # Main content - sử dụng tabs cho các phần
        if not st.session_state.test_completed:
            # Tabs cho các phần
            ...
            with col2:
                if st.button("🏁 KẾT THÚC BÀI THI", type="primary", use_container_width=True):
                    st.session_state.test_completed = True
                    st.rerun()
